"""
Evaluate Exp 2 (simulation-budget sweep): C2ST vs reference at distance bins.
Loads posteriors from results/exp2-nsims/, caches samples, one plot per dim.
"""
...
NSIMS_MARKER = {500: 'o', 1000: 's', 5000: '^', 10000: 'D'}
...
def evaluate_cell(dim, n_sims, models_root, cache_root, n_posterior_samples, n_per_radius,
    ...
    test_points, distance_bins = evaluator.create_test_points(n_points_per_radius=n_per_radius)

    cache_dir  = os.path.join(cache_root, f'dim{dim}', f'n{n_sims}')
    os.makedirs(cache_dir, exist_ok=True)
    cache_path = os.path.join(cache_dir, 'posterior_samples.pkl')

    if os.path.exists(cache_path):
        print(f'  [dim={dim} n={n_sims}] Loading cached samples')
        with open(cache_path, 'rb') as f:
            results = pickle.load(f)
    else:
        posterior_dict = {}
        for name in PROPOSALS:
            pkl = os.path.join(models_root, f'dim{dim}', f'n{n_sims}', name, 'posterior.pkl')
...
def plot_dim(dim, dim_c2st, out_path):
    ...
    for ax_i, n_sims in enumerate(N_SIMS_LIST):
        ax = axes_flat[ax_i]
        c2st_n = dim_c2st.get(n_sims, {})
        for i, pname in enumerate(PROPOSALS):
            data = c2st_n.get(pname, {})
            if not data:
                continue
    ...
    for j in range(len(N_SIMS_LIST), len(axes_flat)):
        axes_flat[j].set_visible(False)

    for ax_i in range((nrows - 1) * ncols, nrows * ncols):
        if ax_i < len(N_SIMS_LIST):
            axes_flat[ax_i].set_xlabel('Distance from prior centre')

    fig.suptitle(f'C2ST vs distance — simulation budget sweep (D={dim})', fontsize=11)
    plt.tight_layout()
...
def main():
    ...
    if args.models_root is None:
        args.models_root = os.path.join(ROOT, 'results', args.task_name, 'exp2-nsims')
    ...
    if os.path.exists(results_path):
        ...
        for dim in DIMS:
            ...
            for n_sims in N_SIMS_LIST:
                print(f'\n=== dim={dim}  n_sims={n_sims} ===')
                all_c2st[dim][n_sims] = evaluate_cell(
                    dim, n_sims, args.models_root, cache_root,
    ...
    for dim in DIMS:
        plot_dim(dim, all_c2st[dim],
                 os.path.join(args.out_root, f'plot_c2st_vs_nsims_dim{dim}.pdf'))
